...
while ~feof(fid)
    ...
    elseif regexp(tline, '^MSG')
        ...
        if regexp(tline, '!MODE RECORD CR')
            ...
            % chernandez: These 2 lines generate the error "Undefined function 'tokenize' for input arguments of type 'char'."
            % then I replace them with these:
            tok=regexp(tline,' ','split');
            asc.fsample=str2double(tok{1,5});
        end
